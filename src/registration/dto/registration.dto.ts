...
export class RegistrationDto {
    ...
    @ApiProperty({
        example: 'a@b.com',
        description: 'Unique Email Id of User',
        uniqueItems: true,
        required: true,
    })
    email: string
    ...
    @MinLength(6, {
        message: (args: ValidationArguments) => {
            if (args.value.length < 6) {
                throw new BadRequestException(
                    `${args.property} length should be min 6`,
                )
            } else {
                throw new InternalServerErrorException()
            }
    ...
    @MaxLength(13, {
        message: (args: ValidationArguments) => {
            if (args.value.length > 13) {
                throw new BadRequestException(
                    `${args.property} length should be max 13`,
                )
            } else {
                throw new InternalServerErrorException()
            }
    ...
    @ApiProperty({
        example: '123456',
        description: 'Min 6, Max 13 length password of user',
        required: true,
    })
    password: string
